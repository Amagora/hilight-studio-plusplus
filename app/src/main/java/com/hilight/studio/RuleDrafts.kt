...
object RuleDrafts {
    fun nextWholeAppRule(
        pkg: String,
        label: String,
        existing: List<AppRule>,
    ): AppRule? = com.hilight.studio.nextWholeAppRule(pkg, label, existing)

    fun copyWholeAppRule(
        source: AppRule,
        targetPkg: String,
        targetLabel: String,
    ): AppRule = com.hilight.studio.copyWholeAppRule(source, targetPkg, targetLabel)

    fun replacesExistingRule(
        existing: List<AppRule>,
        candidate: AppRule,
        openedRule: AppRule,
        isNew: Boolean,
    ): Boolean = com.hilight.studio.replacesExistingRule(existing, candidate, openedRule, isNew)
}
