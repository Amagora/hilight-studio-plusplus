...
public final class Engine {
    ...
    private boolean tick(long elapsedRealtime, boolean requireRunning) {
        synchronized (lock) {
            // stop() may have closed the session after the loop condition was sampled.
            if (requireRunning && !running) return false;
            ...
            int[] frame = renderer.frame(cfg, t, Math.max(1, lights.ledCount()), overdrive);
            ...
            if (!lights.isSessionOpen() || priority != lights.sessionPriority()) {
                if (lights.isSessionOpen() && !lights.closeSession()) return false;
                if (!lights.openSession(priority)) return false;
            }
            lastVisibleAttemptGeneration = stateGeneration;
            lastVisibleConfig = cfg;
            if (!lights.push(output)) return false;
            markSettled();
            return true;
        }
    }
    ...
    private int[] protect(int[] frame, long elapsedRealtime) {
        return safety.apply(frame, elapsedRealtime, dim, overdrive);
    }
    ...
    private void noteDark(long elapsedRealtime) {
        safety.apply(BLANK, elapsedRealtime, dim, overdrive);
    }
