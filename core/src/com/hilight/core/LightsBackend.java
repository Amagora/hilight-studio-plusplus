...
public final class LightsBackend {
    ...
    public void connect() throws Exception {
        ...
        Method setBrightness = null;
        try {
            setBrightness = LightState.Builder.class.getMethod("setBrightness", int.class);
        } catch (Throwable ignored) {}
        final Method finalSetBrightness = setBrightness;
        ...
        io = new SessionIo() {
            ...
            public boolean set(int[] lightIds, int[] colors) throws Exception {
                ...
                for (int i = 0; i < lightIds.length; i++) {
                    ...
                    int finalColor = (c & 0x00FFFFFF) == 0 ? 0 : (c | 0xFF000000);
                    LightState.Builder b = new LightState.Builder().setColor(finalColor);
                    if (finalSetBrightness != null && (c & 0x00FFFFFF) != 0) {
                        try {
                            finalSetBrightness.invoke(b, 255);
                        } catch (Throwable ignored) {}
                    }
                    states[i] = b.build();
                }
