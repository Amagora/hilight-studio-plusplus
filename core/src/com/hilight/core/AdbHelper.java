...
public final class AdbHelper {
    ...
    static final class SingletonClaim implements AutoCloseable {
        ...
        private static void makeCrossUidWritable(Path path) throws IOException {
            ...
            PosixFileAttributeView view = Files.getFileAttributeView(
                    ...
                    LinkOption.NOFOLLOW_LINKS);
            if (view == null) return; // Non-POSIX filesystem (e.g. host unit tests on Windows)
            try {
    ...
    private void run() throws Exception {
        ...
        try {
            ...
            while (true) {
                ...
                boolean changed = reloadIfChanged(now);
                if (changed || now - lastStatus >= STATUS_MS) {
                    lastStatus = now;
                    writeStatus(now);
                }
                Thread.sleep(changed ? 50 : 250);
            }
        } finally {
    ...
    private Object cachedPm = null;
    private Method cachedIsSuspendedMethod = null;
    private Method cachedIsStoppedMethod = null;
    private boolean pmLookupAttempted = false;
    private long lastOwnerCheckMs = 0;
    private boolean lastOwnerSuspendedOrStopped = false;
    private boolean ownerSuspendedApplied = false;

    private boolean isOwnerSuspendedOrStopped(long now) {
        if (owner == null || owner.isEmpty()) return false;
        if (now - lastOwnerCheckMs < 2000) {
            return lastOwnerSuspendedOrStopped;
        }
        lastOwnerCheckMs = now;
        if (!pmLookupAttempted) {
            pmLookupAttempted = true;
            try {
                ...
                if (b != null) {
                    ...
                    cachedPm = stubClass.getMethod("asInterface", IBinder.class).invoke(null, b);
                    if (cachedPm != null) {
                        try {
                            cachedIsSuspendedMethod = cachedPm.getClass().getMethod("isPackageSuspendedForUser", String.class, int.class);
                        } catch (NoSuchMethodException ignored) {}
                        try {
                            cachedIsStoppedMethod = cachedPm.getClass().getMethod("isPackageStoppedForUser", String.class, int.class);
                        } catch (NoSuchMethodException ignored) {}
                    }
                }
            } catch (Throwable ignored) {}
        }
        if (cachedPm == null) return false;
        try {
            if (cachedIsSuspendedMethod != null) {
                Boolean suspended = (Boolean) cachedIsSuspendedMethod.invoke(cachedPm, owner, 0);
                if (Boolean.TRUE.equals(suspended)) {
                    lastOwnerSuspendedOrStopped = true;
                    return true;
                }
            }
            if (cachedIsStoppedMethod != null) {
                Boolean stopped = (Boolean) cachedIsStoppedMethod.invoke(cachedPm, owner, 0);
        ...
        lastOwnerSuspendedOrStopped = false;
        return false;
    }
     ...
     *
     * Directly compares raw content so coarse timestamp resolution on Android storage can never
     * swallow a manual stop or state update.
     */
    private static final String IDLE_STATE =
            "{\"v\":2,\"enabled\":false,\"arm\":false,\"privacyObserverEnabled\":false,\"privacyOutputEnabled\":false}";

    private boolean reloadIfChanged(long now) {
        boolean suspendedOrStopped = isOwnerSuspendedOrStopped(now);
        if (suspendedOrStopped) {
            if (!ownerSuspendedApplied) {
                ownerSuspendedApplied = true;
                lastRaw = null;
                engine.setState(IDLE_STATE);
            }
            return false;
        }
        if (ownerSuspendedApplied) {
            ownerSuspendedApplied = false;
            lastRaw = null;
        }
        String raw = read(stateFile);
        if (raw == null || raw.isEmpty()) return false;
        if (raw.equals(lastRaw)) return false;
        lastRaw = raw;
    ...
    private String lastWrittenStatus = null;
    ...
    private void writeStatus(long now) {
        ...
        try {
            ...
            if (statusStr.equals(lastWrittenStatus)) return;
            lastWrittenStatus = statusStr;
            byte[] data = statusStr.getBytes(StandardCharsets.UTF_8);
