# Hilight-Studio-PlusPlusV3 (Build `a1.3.0` / v3.9.1) - Changelog & Modifications Log

This document provides a comprehensive record of all custom modifications, architectural enhancements, user directives, and privacy protections implemented in **Hilight-Studio-PlusPlusV3** (version `a1.3.0` / v3.9.1, incorporating upstream [DhananjayBhosale/hilight-studio](https://github.com/DhananjayBhosale/hilight-studio) v1.0.9 stuck LED mitigations, safety overrides, zero wake-lock battery optimization, and on-device AI system isolation).
...
| **16** | *"When an app is opened the notification LED starts like intended but it doesn't necessarily stop either. It triggers randomly or it triggers when the screen is locked or goes to AOD mode."* | 1) ForegroundWatcher replayed overlapping past events into persistent map, resurrecting paused apps. 2) Pause events with null/different class names failed to remove app from tracker. 3) Missing interactivity and lock checks caused apps to remain foregrounded during screen off/lock. 4) `ACTION_SCREEN_OFF` auto-armed ambient timer with `armOnRelease = true` and re-sent lingering foreground alerts. | **Foreground & Screen Lock / AOD Lifecycle Fix**: 1) Monotonic event timestamp tracking in `ForegroundWatcher` without duplicate replay. 2) Enhanced `ForegroundAppTracker` to clear all package entries on pause/stop with null/blank class names and added `STOPPED` lifecycle handling. 3) Added `PowerManager.isInteractive` and `KeyguardManager.isKeyguardLocked` checks to clear foreground state when locked/screen off. 4) Changed `ACTION_SCREEN_OFF` to clear foreground overrides and cancel alerts without auto-arming (`armOnRelease = false`). |
| **17** | *"Incorporate original dev's upstream fixes (v1.0.9 stuck LED & lifecycle hardening) into fork without breaking custom features, and bump to v3.6."* | Hardware LED latching/stuck pixels on Pixel 11 Pro after abrupt exits, plus daemon collision between Shizuku, ADB, and Root backends. | **Upstream Merge & Hardening (v3.6 / `a1.2.6`)**: 1) Merged `RendererContract` protocol v1 / revision 4 / schema 6. 2) 3-pass bounded black clear sequences (`ForcedBlackClear`, `LightsBackend`). 3) Instance-fenced ADB, Shizuku, and Root handoffs with SIGTERM `/proc` validation. 4) Privacy-safe diagnostic status snapshots (`RendererDiagnostics`) and manual LED cleanup retry in Setup tab. 5) Preserved all custom fork features (overdrive, 3-color transitions, 8 per-LED arrays, While-Open holds, desktop manager, Material 3 palettes). |
| **18** | *"Ensure Disable Taper and Max Brightness Override operate independently, verify zero wake-lock/battery drain loops, respect app timers and force-stop, guarantee full 8-LED hardware brightness, ensure zero conflict with on-device AI apps (Android System Intelligence / AI Core), and bump to v3.9 (`a1.2.9`)."* | 1) Need clear separation of Test tab "Disable Taper" vs Setup tab global "Max Brightness Override". 2) Battery drain/wake lock loop prevention during screen off. 3) Digital Wellbeing/app timer and force-stop compliance. 4) Modern Android HAL 100% luminance drive via `LightState.Builder.setBrightness(255)`. 5) Avoid false light flashes from background AI telemetry pings. | **V3.9 (`a1.2.9`) Feature & Stability Release**: 1) Separated "Disable Taper" on Test from "Max Brightness Override" in Setup; Setup override automatically locks Test toggle ON with warning alert. 2) Screen-off interactive suppression pauses background tick timers completely (0 wake locks, 0 background CPU loops). 3) `isOwnerSuspendedOrStopped` detects Android Settings force-stop via reflected `IPackageManager` and drops hardware session immediately. 4) `LightState.Builder.setBrightness(255)` drives full-current hardware output across all 8 LEDs with 100% saturated color channels. 5) `INTERNAL_SYSTEM_OR_AI_PKGS` filter prevents internal daemons (`com.google.android.as`, `com.google.android.aicore`, `android`, etc.) from triggering Catch-All rules. |
| **19** | *"1. AMOLED mode isn't truly AMOLED mode (needs pure black with darker buttons/panels and dynamic evening hours support). 2. Make foldable dock movable right/left when unfolded. 3. In-app persistent notification toggle fix. 4. Capture authentic on-device AMOLED screenshots folded and unfolded for README."* | 1) Dark theme was using `#121216` dark gray instead of pure `#000000` OLED black. 2) Unfolded navigation rail was fixed to left side. 3) Notification persistence toggle in Setup did not stop the running service. 4) Need real on-device screenshots of all 4 tabs in AMOLED pitch black and unfolded wide screen. | **True AMOLED Pitch Black, Foldable Dock Positioning & Notification Sync**: 1) Converted root Scaffold, TopAppBar, NavigationBar, and NavigationRail to pure `#000000` with `#050507`–`#0F0F13` elevated panels, plus dynamic schedule toggle in Setup. 2) Added **"Foldable dock position (unfolded)"** selector (`Left` \| `Right`) in Setup $\rightarrow$ Appearance. 3) Connected `setPersistentNotificationEnabled()` to `syncForegroundWatcher()` with live `stopForeground(STOP_FOREGROUND_REMOVE)` and `startForeground()` dynamic switching. 4) Captured 8 authentic on-device screenshots via ADB (4 folded AMOLED tabs + 4 unfolded inner display views) and updated `README.md`. |
| **20** | *"1. Rework the UIs for adjusting LEDs and settings with uniform elements. 2. Expand spacing and utilize more screen space when selecting an app or adjusting light configurations. 3. Add manual numeric entry option when tapping slider values. 4. Clean up app names (no package IDs like `com.appname.app`). 5. Remove persistent notification entirely. 6. Bump version to v3.9.1 / `1.3.0`."* | 1) Small, cramped dialogs on phone/foldable screens. 2) Tedious slider thumb dragging to hit exact numbers. 3) Package names shown as subtitles in search pickers and cards. 4) Lingering foreground notification in shade. | **UI Expansion, Manual Numeric Entry, Clean App Names & Notification Elimination (v3.9.1 / `a1.3.0`)**: 1) **Direct Manual Numeric Input**: Tapping any slider value badge opens `SliderValueEditDialog` with custom parser (`DurationInput.kt`) supporting direct numbers, percentages, durations (`2.5s`), and Hue degrees (`0°..359°`) with quick preset chips. 2) **Expansive Full-Height Dialogs**: Converted `AppPickerDialog` (90%), `RuleEditorDialog` (94%), and `PrivacyRuleEditorDialog` (94%) to modern full-height modals with Live LED Hero cards and uniform `PixelCard` groups. 3) **Clean App Names & Label Resolver**: Built `AppNames.kt` resolving human titles via `PackageManager` with fallback title casing; eliminated package subtitles from search lists and rule cards. 4) **Zero Ongoing Notification**: Replaced persistent notification with silent startup disposal (`stopForeground(STOP_FOREGROUND_REMOVE)` + startup cancel) while maintaining full service reliability. 5) Bumped version to **v3.9.1 / `a1.3.0` (`versionCode 130`)**. |
